...
### 4.4.6 RasterizeGaussians 逐阶段细解（重点）

下面用“GPU 路径”的思维模型解释 `RasterizeGaussians::apply(...)` 的关键阶段（CPU 版本逻辑等价，只是实现方式不同）：

1. **Tile 分桶（binning）**  
   - 输入的 `numTilesHit` 告诉系统每个高斯会覆盖哪些 tile。  
   - 高斯会被分配到对应 tile 的候选列表中，避免“每个像素遍历全部高斯”的 O(P×N) 代价。

2. **Tile 内候选排序（通常按深度）**  
   - 对同一 tile 内可能贡献的高斯，按深度或可见顺序组织。  
   - 目标是让前后遮挡与 alpha 合成稳定，减少错误叠加。

3. **像素级椭圆评估**  
   - 对 tile 中每个像素，读取高斯的 `xys + conics + radii`。  
   - 利用二次型快速判断“像素是否在该高斯有效影响域内”。  
   - 若超出 `radii`/阈值范围则直接跳过（早停剪枝）。

4. **权重与颜色贡献计算**  
   - 对有效像素，基于 conic 距离算出高斯核权重 `w`。  
   - 将 `w` 与 `alpha = sigmoid(opacity)` 结合，得到该高斯对该像素的真实贡献强度。  
   - 颜色贡献约为 `w * alpha * rgb_gaussian`（概念式，具体实现会做数值稳定处理）。

5. **前向 alpha 合成（front-to-back compositing）**  
   - 维护像素累计颜色 `C_acc` 与透过率/剩余透射 `T`。  
   - 每加入一个高斯贡献：  
     - `C_acc += T * alpha_i * color_i`  
     - `T *= (1 - alpha_i)`  
   - 当 `T` 很小（像素近乎不透明）可提前终止该像素后续累积。

6. **背景融合与输出**  
   - 像素最终颜色：`C_out = C_acc + T * backgroundColor`。  
   - 输出张量形状为 `H x W x 3`，随后在 `forward` 末尾再 `clamp_max(1.0)`。

### 4.4.7 为什么这种实现快

- **空间稀疏化**：tile 分桶将全局遍历降为“局部候选遍历”。
- **计算剪枝**：radii / conic 阈值快速排除无贡献像素-高斯对。
- **早停机制**：高 alpha 像素会较早达到不透明，减少后续运算。
- **并行友好**：tile 与像素天然适合 GPU 并行 kernel。

### 4.4.8 与反向传播的对应关系

`RasterizeGaussians` 不只是前向渲染器，它还决定梯度如何传到：

- `xys/conics/radii`（几何投影相关）
- `rgbs`（SH 颜色相关）
- `opacities`（透明度相关）

这也是为什么训练中“投影算子 + 光栅化算子”必须成对理解：前者决定几何落点，后者决定像素混合和梯度分配路径。
